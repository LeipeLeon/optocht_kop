module FuKing
  ...
  class Twitter
    ...
    def self.read_status(params = {})
      url = params[:read_url] || configs[:read_url]
      doc = Hpricot.XML(do_get(url,params))
      parse_response(doc)
    end

    def self.update(status, params = {})
      in_reply_to_status_id = params['in_reply_to_status_id'] || nil
      url = params[:write_url] || configs[:write_url]
      ...
      if configs["#{RAILS_ENV}_uri".to_sym].blank?
        ...
        response = do_post(url, :status => CGI::escapeHTML(status), :in_reply_to_status_id => in_reply_to_status_id)
